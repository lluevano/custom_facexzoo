#ShuffleFaceNet Pytorch implementation
#Modified from from https://github.com/mbfaria/ShuffleFaceNet_Pytorch/blob/master/core/model.py
...
class ShuffleFaceNet(nn.Module):
    def __init__(self, feat_dim=128, multiplier=1.5, image_size=112, inverted_residual=InvertedResidual):
        ...
        #stages_repeats = [4, 8, 4] from older code
    ...
    def _forward_impl(self, x):
        ...
        x = nn.functional.interpolate(x, size=self.image_size)
        x = self.conv1(x)
        # x = self.maxpool(x)
        x = self.stage2(x)
        x = self.stage3(x)
        ...
        x = x.view(x.size(0), self._stage_out_channels[-1], 1)
        x = self.linearconv(x)
        x = x.view(x.size(0), self.feat_dim, 1, 1)
        x = self.bn(x)
        x = x.view(x.size(0), -1)
